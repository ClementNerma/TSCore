...
export class Dictionary<K, V> implements Iterable<[K, V]> {
  ...
  constructor(content?: Array<[K, V]> | Map<K, V> | Dictionary<K, V>) {
    this._content =
      content instanceof Map
        ? content
        : content instanceof Dictionary
        ? new Map(content._content)
        : new Map(content)
  }
  ...
  static fromObject<T extends object>(
    object: T
  ): Dictionary<Exclude<keyof T, number>, T[Exclude<keyof T, number>]> {
  ...
  mapKeys<X>(mapper: (key: K, value: V) => X): Dictionary<X, V> {
    return new Dictionary(
      Array.from(this._content.entries()).map(([key, value]) => [mapper(key, value), value])
    )
  }
  ...
  mapValues<Y>(mapper: (value: V, key: K) => Y): Dictionary<K, Y> {
    return new Dictionary(
      Array.from(this._content.entries()).map(([key, value]) => [key, mapper(value, key)])
    )
  }
  ...
  map<X, Y>(mapper: (key: K, value: V) => [X, Y]): Dictionary<X, Y> {
    return new Dictionary(
      Array.from(this._content.entries()).map(([key, value]) => mapper(key, value))
    )
  }
   ...
   * @param mapper
   */
  mapKeysToCollection<X extends string | number | symbol>(
    mapper: (key: K, value: V) => X
  ): { [S in X]: V } {
   ...
   * @param mapper
   */
  mapValuesToCollection<Y>(
    mapper: (value: V, key: K) => Y
  ): Result<DictionaryToCollection<K, Y>, void> {
    if (this.size == 0) {
    ...
    const mappedEntries: Array<[K, Y]> = Array.from(this._content.entries()).map(([key, value]) => [
      key,
      mapper(value, key),
    ])
  ...
  mapValuesToCollectionUnchecked<Y>(mapper: (value: V, key: K) => Y): DictionaryToCollection<K, Y> {
    const mappedEntries = Array.from(this._content.entries()).map(([key, value]) => [
      key,
      mapper(value, key),
    ])
   ...
   * @param mapper
   */
  mapToCollection<X extends string | number | symbol, Y>(
    mapper: (key: K, value: V) => [X, Y]
  ): { [S in X]: Y } {
    const mappedEntries = Array.from(this._content.entries()).map(([key, value]) =>
      mapper(key, value)
    )
   ...
   */
  resultable<X, Y, E>(
    tester: (key: K, value: V, dict: this) => Result<[X, Y], E>
  ): Result<Dictionary<X, Y>, E> {
   ...
   * @param tester
   */
  resultableValues<U, E>(
    tester: (key: K, value: V, dict: this) => Result<U, E>
  ): Result<Dictionary<K, U>, E> {
