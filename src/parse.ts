    ...
    rawLines(): Array<DecodingErrorLine> {
        return this.match({
            ...
            NoneOfEnumStates: (err): DecodingErrorLine[] =>
                [['f', `Value is not one of enumeration "{}"'s state: {}`, [err[0], err[1].join(', ')]]],
...
export namespace Decoders {
    ...
    export function untypedMapped<F>(mappings: Array<[string, Decoder<F, unknown>]>): Decoder<Collection<F> | Dictionary<string, F>, { [key: string]: unknown }> {
        return encoded => {
    ...
    export function oneOf<F, T extends F>(candidates: T[]): Decoder<F, T> {
        return value => {
            if (candidates.includes(value as any)) {
                ...
                return Err(new DecodingError(state('NoneOfCases', candidates.map(c => _stringify(c)))));
            }
        };
    }
    ...
    export function cases<K extends string | number | symbol, T>(cases: { [key in K]: T }): GDecoder<T> {
        return then(string, value => {
            ...
            return Err(new DecodingError(state('NoneOfCases', O.keys(cases).map(entry => entry.toString()))));
        });
    }
