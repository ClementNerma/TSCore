...
export class Iter<T> extends AbstractMatchable<IterState> implements Iterable<T> {
  ...
  consume<U>(
    mapper: (value: T, index: number, iterator: this) => U,
    toList = false
  ): List<U> | void {
 ...
 * @param object
 */
export function iter<T extends object>(
  object: T
): Iter<[Exclude<keyof T, number>, T[Exclude<keyof T, number>]]> {
