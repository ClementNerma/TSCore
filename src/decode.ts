...
> {
  ...
  rawLines(): Array<DecodingErrorLine> {
    return this.match<DecodingErrorLine[]>({
      ...
      NoneOfCases: (candidates) => [
        ["f", `Value is not one of the candidate values: {}`, [candidates.join(", ")]],
      ],

      NoneOfEnumStates: ([enumName, states]) => [
        ["f", `Value is not one of enumeration "{}"'s state: {}`, [enumName, states.join(", ")]],
      ],
      ...
      switch (line[0]) {
        ...
        case "e":
          rendered = rendered.concat(
            DecodingError.renderLines(line[1].rawLines()).map((line) => "\t" + line)
          )
          break
...
export namespace Decoders {
  ...
  export const exactlyNull: Decoder<unknown, null> = (value) =>
    value === null ? Ok(null) : Err(new DecodingError(state("WrongType", "null")))
  ...
  export const dictionary: Decoder<unknown, Dictionary<unknown, unknown>> = (value) =>
    value instanceof Dictionary
      ? Ok(value)
      : Err(new DecodingError(state("WrongType", "Dictionary")))

  /** Decode records */
  export const record: Decoder<unknown, RecordDict<unknown>> = (value) =>
    value instanceof RecordDict
      ? Ok(value)
      : Err(new DecodingError(state("WrongType", "RecordDict")))
  ...
  export function listOf<T>(decoder: GDecoder<T>): GDecoder<List<T>> {
    return then(instanceOf(List), (list) =>
      list.resultable((item, i) =>
        decoder(item).mapErr((err) => new DecodingError(state("ListItem", [i, err])))
      )
    )
  }
    ...
    return then(instanceOf(Dictionary), (dict) =>
      dict.resultable((key, value) =>
        keyDecoder(key)
          .mapErr(
            (err) =>
              new DecodingError(state("DictionaryKey", [stringify(key, { prettify: false }), err]))
          )
          .andThen((key) =>
            valueDecoder(value)
              .mapErr(
                (err) =>
                  new DecodingError(
                    state("DictionaryValue", [stringify(key, { prettify: false }), err])
                  )
              )
              .map((value) => [key, value])
          )
        ...
        .resultable((key, value) =>
          string(key)
            .mapErr(
              (err) =>
                new DecodingError(
                  state("DictionaryKey", [stringify(key, { prettify: false }), err])
                )
            )
            .andThen((key) =>
              valueDecoder(value)
  ...
  export function instanceOf<F, T>(cstr: new (...args: any[]) => T): Decoder<F, T> {
    return (value) =>
      value instanceof cstr
        ? Ok(value)
        : Err(new DecodingError(state("WrongType", `constructor[${cstr.name}]`)))
  }

  /** Sub-type a value to a primitive type */
  export function typedPrimitive<P extends null | boolean | number | string>(
    primitive: P
  ): Decoder<unknown, P> {
    return (value) =>
      value === primitive
        ...
        : Err(
            new DecodingError(
              state("WrongType", `primitive[${stringify(primitive, { prettify: false })}]`)
            )
          )
  }
  ...
  export function withType<F, T extends F>(
    ...
    return (value) =>
      predicate(value) ? Ok(value) : Err(new DecodingError(state("WrongType", typename)))
  }
        ...
        : Err(
            new DecodingError(
              state("NoneOfEnumStates", [
                cstr.name,
                cases.map((c) => stringify(c, { prettify: false })),
              ])
            )
          )
  }
  ...
  export function untypedCombine<F, T>(
    decoders: Array<Decoder<F, unknown>>,
    merger: (...values: unknown[]) => T
  ): Decoder<F, T> {
    return (value) => {
      ...
      for (const [i, decoder] of decoders.entries()) {
        ...
        if (decoded.isErr())
          return Err(new DecodingError(state("FailedCombining", [i, decoded.err])))
  ...
  export function untypedMapped<F>(
    mappings: Collection<Decoder<F, any>>
  ): Decoder<CollLike<F>, Collection<unknown>> {
    return (encoded) => {
  ...
  export function mapped<F, O extends object>(
    decoders: { [key in keyof O]: Decoder<unknown, O[key]> }
  ): Decoder<CollLike<F>, O> {
