...
export class O {
   ...
   * @param object
   */
  static strKeys<T extends object>(
    object: T
  ...
  static entries<T extends object>(
    object: T
  ): Array<[Exclude<keyof T, number>, T[Exclude<keyof T, number>]]> {
   ...
   * @param mapper
   */
  static mapValues<T extends object, Y>(
    object: T,
    mapper: (key: keyof T, value: T[keyof T]) => Y
  ): { [S in keyof T]: Y } {
    ...
    return new Proxy(obj as any, {
      get(_, prop) {
        return obj.hasOwnProperty(prop)
          ? obj[prop as Exclude<keyof T, number>]
