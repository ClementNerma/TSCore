 ...
 */
export function deepCompareEq(
  left: unknown,
  right: unknown,
  _ctx: string[] = []
): Result<true, FailedEqCmp> {
  ...
  if (O.isCollection(left) && O.isCollection(right)) {
    ...
    for (const key of leftKeys) {
      ...
      const test = deepCompareEq(
        (left as any)[key],
        (right as any)[key],
        _ctx.concat(("Key " + key) as string)
      )
  ...
  if (Result.is(left) && Result.is(right)) {
    ...
    return left.isOk()
      ...
      : deepCompareEq(
          left.maybeErr().unwrap(),
          right.maybeErr().unwrap(),
          _ctx.concat("[Result].Err")
        )
  }
...
export function assertIs(
  ...
  if (strict ? left !== right : left != right) {
    panic(
      ...
      panicMessage ||
        format(
          "Assertion failed (Left and right values are not {}identical)",
          strict ? "strictly " : ""
        )
    )
  }
