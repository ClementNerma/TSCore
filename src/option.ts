...
abstract class OptionClass<T> extends AbstractMatchable<OptionMatch<T>> {
   ...
   * @param mapper A callback returning a nullable value from this option's concrete value
   */
  abstract andThenMaybeAsync<U>(
    mapper: (value: T) => Promise<U | null | undefined>
  ): Promise<Option<U>>
...
export namespace Option {
  ...
  export async function maybeAsync<T>(
    futureValue: Promise<T | null | undefined>
  ): Promise<Option<T>> {
  ...
  export function expectSome<T>(value: T | null | undefined, message?: string): Option<T> {
    return Some(
      value ?? panic(message || "Tried to use a null or undefined value as non-nullable!")
    )
  }
