...
export interface TSCoreEnv {
  ...
  verbosity: (
    devMode: boolean,
    context: Exclude<FormattingContext, "format" | "logging" | "panic">
  ) => boolean
...
export interface FormatOptions {
  ...
  ) => StringifierOptions & StringificationOptions
}
...
const _tsCoreEnv: { ref: TSCoreEnv } = {
  ref: {
    ...
    defaultFormattingOptions: () => ({
      ...
      stringifyOptions(devMode, context, prettify) {
        return {
          ...
          prettify: prettify || devMode || undefined,
        }
      },
    }),
    ...
    dump(value, options) {
      if (this.devMode()) {
        const baseOptions = this.defaultFormattingOptions().stringifyOptions(
          this.devMode(),
          "dump",
          true
        )
 ...
 */
export type TSCoreEnvUpdater =
  | Partial<TSCoreEnv>
  | ((previousEnv: Readonly<TSCoreEnv>) => Partial<TSCoreEnv>)
...
export function setupTypeScriptCore(envUpdater: TSCoreEnvUpdater): void {
  ...
  for (const [key, value] of O.entries(
    envUpdater instanceof Function ? envUpdater(_tsCoreEnv.ref) : envUpdater
  )) {
    // @ts-ignore
    out[key] = value
  }
 ...
 */
export function formatAdvanced(
  message: string,
  params: unknown[],
  context: FormattingContext,
  maybeOptions?: FormatOptions
): string {
  ...
  return message.replace(/{([a-zA-Z0-9_:!#\?\$]*)}/g, (match, format) => {
    ...
    const supported = new Regex(/^(\d+)?([:!#])?([bdoxX])?(\?)?$/, [
      "strParamPos",
      "display",
      "numberFormat",
      "pretty",
    ...
    if (display === "!" || display === "#" || !display) {
      ...
      const stringifyOptions: StringifierOptions & StringificationOptions = {
        ...
        highlighter:
          (context === "format" || context === "logging") &&
          _tsCoreEnv.ref.disableHighlighterInFormatContext(true)
            ? undefined
