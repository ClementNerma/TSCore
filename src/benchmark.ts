 ...
 */
export function comparativeBenchmark(content: ComparativeBenchmarkContent, timeout = 5000, waitPromises = true): Future<ComparativeBenchmarkResult, void> {
    return new Future(async resolve => {
        ...
        resolve(averages.mapValues(average => ({
            average,
    ...
    for (const [name, result] of results) {
        ...
        for (const [otherName, relative] of result.relative) {
            if (name === otherName) {
                ...
                row.push(percentage ? '+ ' + numFormatter((relative - 1) * 100) + '%'
                                    : numFormatter(relative) + ' x faster');
            } else {
                row.push(percentage ? '- ' + numFormatter(100 - (100 * relative)) + '%'
                                    : numFormatter(1 / relative) + ' x slower');
            }
        }
